...
(define dbnlexer
  (lexer-src-pos
   ...
   [(:or "PAPER" "Paper")                           (token-PAPER)] ; Added by Lexxi
   [(:or "PEN" "Pen")                               (token-PEN)] ; Added by Lexxi
   [(:or "LINE" "Line")                             (token-LINE)] ; Added by Lexxi
   [(:or "SET" "Set")                               (token-SET)] ; Added by Lexxi
   ...
   ; Added by Lexxi
   ; (:+ numeric) is a regular expression for all numeric values
   ; Convert the number 'num' to a string and check if it is a valid integer [0-9]+
   ; If the number is valid, return the number, else give an error
   [(:+ numeric) (token-NUMERICVALUE
                  (let ([num (string->number lexeme 10 'number-or-false)])
                    (if num
                        num
                        (raise-lex-error "Error: Invalid Number" start-pos lexeme))))]

   ; Added by Lexxi 
   [(:: (:+ (:or lower-case upper-case numeric #\_ #\- #\/)) ".dbn") (token-FILENAME lexeme)]
   
   ; identifiers
   ;;; TODO: Add identifiers, which should be a token-IDENTIFIER and contain the lexeme

   ; Added by Lexxi
   ; Matches values starting with an upper or lowercase letter and may be followed by uppercase letters, lowercase letters, and digits 
    [(:: (:+ lower-case upper-case)(:* (:or numeric lower-case upper-case)))  (token-IDENTIFIER lexeme)]
